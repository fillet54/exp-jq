"""Script and job-output handlers."""
...
def _load_queue_prereqs(
    *,
    ...
    config = ctx.uut_store.get(uut_id)
    if not config:
        return None, None, ("Unknown UUT", 400)
    try:
        config = ctx.uut_store.snapshot(uut_id) or config
        ctx.log.info("Snapshot UUT %s tree=%s", config.name, config.last_tree_sha)
    except Exception as exc:
        ctx.log.exception("Failed to snapshot UUT %s: %s", uut_id, exc)

    try:
        scripts_tree = snapshot_tree(base_path, cache_dir=ctx.scripts_cache_dir)
        ctx.log.info("Snapshot scripts tree at %s -> %s", base_path, scripts_tree)
    except Exception:
        scripts_tree = None
        ctx.log.exception("Failed to snapshot scripts at %s", base_path)
    ...
    for raw_entry in raw_entries:
        for part in raw_entry.replace("\r", "\n").split("\n"):
            ...
            if not candidate:
                continue
            try:
                rel_script_path = script_helpers.resolve_rel_script_path(candidate, base_path)
            except ValueError:
...
@bp.route("/scripts", methods=["GET"], endpoint="scripts_panel")
def scripts_panel() -> str:
    ...
    scripts = script_helpers.discover_scripts(base_path)
    systems, system_counts, system_index = script_helpers.build_script_system_index(scripts)
    (
        directory_nodes,
        directory_children,
        directory_to_scripts,
        _directory_recursive_counts,
    ) = script_helpers.build_script_directory_index(scripts)
    ...
    try:
        ...
        ctx.log.warning("Failed to load default requirements for scripts panel.", exc_info=True)
        requirement_text_map = {}

    systems = script_helpers.sorted_systems(system_index.keys())
    if systems:
        if not selected_system or selected_system not in systems:
            selected_system = systems[0]
    else:
        selected_system = script_helpers.UNSPECIFIED_SYSTEM
...
def script_detail_page(script_relpath: str) -> str:
    ...
    return render_template(
        ...
        script_title=script_helpers.extract_rst_title(script_lines, fallback=Path(script_relpath).stem),
...
def job_output_page(job_id: str) -> str:
    context = script_helpers.build_job_output_context(ctx, job_id)
    if not context:
        abort(404)
...
@bp.route("/jobs/<job_id>/output/panel", methods=["GET"], endpoint="job_output_panel")
...
@bp.route("/jobs/<job_id>/output/raw", methods=["GET"], endpoint="job_output_raw")
...
@bp.route("/jobs/<job_id>/output/artifacts/<path:artifact_path>", methods=["GET"], endpoint="job_output_artifact")
def job_output_artifact(job_id: str, artifact_path: str) -> Any:
    ...
    root = (Path(ctx.central.artifacts_dir).resolve() / job_id).resolve()
    ...
    if not candidate.exists() or not candidate.is_file():
        result = ctx.queue.get_result(job_id) or {}
        worker_address = str(result.get("worker_address") or "").strip()
        if worker_address and hasattr(ctx.central, "_download_artifact"):
            try:
                ctx.central._download_artifact(worker_address, job_id, normalized)
            except Exception:
                pass
    if not candidate.exists() or not candidate.is_file():
        abort(404)
...
@bp.route("/jobs/from_script", methods=["POST"], endpoint="queue_from_script")
def queue_from_script() -> Any:
    script_path = (request.form.get("script_path") or "").strip()
    base_path = Path(request.form.get("base_path") or ctx.scripts_root).resolve()
    ...
    try:
        rel_script_path = script_helpers.resolve_rel_script_path(script_path, base_path)
    ...
    try:
        jobs_to_queue = script_helpers.build_jobs_for_relpath(
            ctx,
        ...
        return str(exc), 400
    if suite_name:
        ctx.suite_manager.create_suite(suite_name)
        ctx.suite_manager.add_script(suite_name, rel_script_path)
...
@bp.route("/jobs/from_scripts", methods=["POST"], endpoint="queue_from_scripts")
def queue_from_scripts() -> Any:
    ...
    try:
        for rel_script_path in rel_script_paths:
            jobs_to_queue.extend(
                script_helpers.build_jobs_for_relpath(
                    ctx,
                    rel_script_path=rel_script_path,
                    base_path=base_path,
                    config=config,
    ...
    if return_to:
        return redirect(return_to, code=303)
    return redirect(url_for("queue.queue_page"), code=303)


@bp.route("/jobs/from_suite", methods=["POST"], endpoint="queue_from_suite")
def queue_from_suite() -> Any:
    ...
    scripts = ctx.suite_manager.get_suite(suite_name)
    if not scripts:
    ...
    for rel_script_path in scripts:
        try:
            clean_rel_script_path = script_helpers.resolve_rel_script_path(rel_script_path, base_path)
        except ValueError:
            continue
        try:
            jobs_to_queue.extend(
