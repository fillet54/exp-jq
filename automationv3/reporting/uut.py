"""UUT configuration persistence and snapshot linkage."""
...
from automationv3.fscache_store import FSCacheStore
...
class UUTStore:
    """Manage UUT configurations in SQLite and snapshot trees via FSCacheStore."""

    def __init__(
        self,
        db_path: str = "jobqueue.db",
        cache_dir: str = ".fscache",
        fscache: Optional[FSCacheStore] = None,
    ) -> None:
        self.db_path = db_path
        self.cache_dir = cache_dir
        self.fscache = fscache or FSCacheStore(default_cache_dir=cache_dir)
        self._init_db()
    ...
    def snapshot(self, uut_id: str) -> Optional[UUTConfig]:
        ...
        if not config:
            return None
        tree_sha = self.fscache.snapshot_tree(config.path, cache_dir=self.cache_dir)
